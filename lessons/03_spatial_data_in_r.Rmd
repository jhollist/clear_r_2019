...
# change these to ! %in% installed.packages
...
While this will work well for point data, it is more likely that people will have other features (lines, polygons, etc.) in an existing spatial file format, such as shapefiles, geopackage, or file geodatabase.  In these cases we can use a differnt `sf` function, `st_read` to read in those files.  We will show an example here of working with a shapefile.  The shapefile is available from <https://github.com/jhollist/clear_r_2019/raw/master/lessons/ct_ri_wbd.zip>.  Download and extract it. Then to read it in:
